...
void prot_login_touch_reply( struct qqclient* qq, qqpacket* p )
{
	...
	if( result == 0x00 ){
		...
		if( test_result == 0 ){
			...
			OPS("### @ NET : Server Redirect to %s", inet_ntoa( addr ) );
...
void prot_login_verify_reply( struct qqclient* qq, qqpacket* p )
{
	...
	case 0x51:	//
		...
		DBG("Denied.");
		OPS("### @ LOGIN : Server Denied.");
		break;
	case 0xBF:
		DBG("No this number.");
		OPS("### @ LOGIN : UID Not Found On Server.");
	case 0x34:
		DBG("Wrong password.");
...
void prot_login_get_info_reply( struct qqclient* qq, qqpacket* p )
{
	...
	qq->server_time = get_int( buf );
	// get time diff from local to server
	qq->local_time_movs = time(NULL) - qq->server_time;
	OPS("### @ LOGIN : Server Time Movs %d", qq->local_time_movs);
	...
	OPS("### @ LOGIN : %d is Online.", qq->self->number);
//	prot_login_a4( qq );
...
//	hex_dump( buf->data, buf->len );
	...
	*(uint*)data = htonl( qq->number );
	...
	OPS("### @ LOGIN : Last Login : %s", ctime( &t ) );
