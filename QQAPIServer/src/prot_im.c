...
//好友修改签名
static void process_buddy_modify_signiture( struct qqclient* qq, qqpacket* p, qqmessage* msg )
{
	bytebuffer *buf = p->buf;
	get_int( buf );//00000000
	msg->from = get_int( buf );//qq
	msg->msg_time = get_int( buf );//modify time
	get_byte( buf );//len
	char sign[256];
	get_string( buf, sign, 255 );//sign
	buddy_sign_callback( qq, msg->from, msg->msg_time, sign );
}
