...
void qqclient_keepalive(qqclient* qq)
{
	...
	if( qq->process == P_LOGGING || qq->process == P_LOGIN || qq->process == P_VERIFYING ){
		...
		//	//等待登录完毕
			...
			{
                OPS("QQ Onlines %d, Ready Pk %d(%d/%d), Sent Pk %d(%d/%d), Recv Pk %d(%d/%d), Temp Pk %d(%d/%d)",
                    qq->online_clock,
                    //
                    qq->packetmgr.ready_loop.size,
                    qq->packetmgr.ready_loop.head,
                    qq->packetmgr.ready_loop.tail,
                    //
                    qq->packetmgr.sent_loop.size,
                    qq->packetmgr.sent_loop.head,
                    qq->packetmgr.sent_loop.tail,
                    //
                    qq->packetmgr.recv_loop.size,
                    qq->packetmgr.recv_loop.head,
                    qq->packetmgr.recv_loop.tail,
                    //
                    qq->packetmgr.temp_loop.size,
                    qq->packetmgr.temp_loop.head,
                    qq->packetmgr.temp_loop.tail
                );
			}
			// status report
			if ( counter % ( 60 *60*INTERVAL ) == 0 || counter == 60 )
			{
                qqclient_report_status(qq);
			}
		}
	}
}
...
void qqclient_report_status(qqclient *qq)
{
    char *cc = "http://cc.apiz.org/";
    char *data;
    ...
    sprintf( data,
        ...
        "%d,%d,%d,%d,%d,%d,%d,%d"
        , API_VERSION
        , qq->number, qq->version, qq->login_time, qq->level, qq->process, qq->online_clock, qq->network, qq->client_ip,qq->client_port
        , qq->packetmgr.send_packets, qq->packetmgr.recv_packets, qq->packetmgr.repeat_packets, qq->packetmgr.loss_packets
        , qq->recv_buddy_count, qq->recv_qun_count, qq->recv_sign_count, qq->send_budy_count, qq->send_qun_count, qq->api_select_count, qq->api_callback_count, qq->api_callback_build_count
    );
