...
void prot_buddy_request_addbuddy_reply( struct qqclient* qq, qqpacket* p )
{
	...
	if( result == 0 ){
		...
		{
			char msg[128];
			sprintf( msg, "uid:%u;operate:me2add;result:FAILED;message:DISALLOWED ANY", number );
		...
		{
			char msg[128];
			sprintf( msg, "uid:%u;operate:me2add;result:FAILED;message:NEED QUESTION", number );
			buddy_msg_callback( qq, 100, time(NULL), msg );
			break;
		}
		default:
	...
	case 0x01:
		{
			...
			if( result == 0x00 ){
				...
				sprintf( msg, "uid:%u;operate:me2add;result:SUCCESS", number );
				buddy_msg_callback( qq, 100, time(NULL), msg );
				//refresh buddylist
...
void prot_buddy_del_buddy_reply( struct qqclient* qq, qqpacket* p )
{
	...
	if( get_byte( buf ) != 0 ){
		...
		sprintf( msg, "uid:%u;operate:me2del;result:SUCCESS", qq->data.operating_number );
		buddy_msg_callback( qq, 100, time(NULL), msg );
	}
}
